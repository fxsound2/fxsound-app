								   ...
								   struct hardwareMeterValType *sp_meters, int DSP_data_type)
{
	...
	static int b_h_init = 0;
	static PT_HANDLE *beat_handle;
	static realtype bassdrum[4096];
	static realtype snaredrum[4096];
	static realtype hihat[4096];

	if( b_h_init == 0 )
	{
		b_h_init = 1;
		if( BeatDspNew(&beat_handle, 0) != OKAY )
			return(NOT_OKAY);
	}
	...
	if( !(s->bypass_on) )
	{
			...
			/* Override user thresholds for now */
#ifdef DSP_READ_VALS
			if( ReadVals.switch_vals[0] )
				BeatDspSetThreshold(beat_handle, BEATDSP_COMPONENT_BASS_DRUM, ReadVals.f_vals[0]);
			else
				BeatDspSetThreshold(beat_handle, BEATDSP_COMPONENT_BASS_DRUM, 1000.0);

			if( ReadVals.switch_vals[1] )
				BeatDspSetThreshold(beat_handle, BEATDSP_COMPONENT_SNARE_DRUM, ReadVals.f_vals[1]);
			else
				BeatDspSetThreshold(beat_handle, BEATDSP_COMPONENT_SNARE_DRUM, 1000.0);

			if( ReadVals.switch_vals[2] )
				BeatDspSetThreshold(beat_handle, BEATDSP_COMPONENT_HIHAT_DRUM, ReadVals.f_vals[2]);
			else
				BeatDspSetThreshold(beat_handle, BEATDSP_COMPONENT_HIHAT_DRUM, 1000.0);
#endif

			if( BeatDspThreePieceProcess(beat_handle,
							(realtype *)lp_data,		/* Input signal, points interleaved */
							l_length,			/* Number of mono sample points or stereo sample pairs */
							2,						/* Number of channels */
							(realtype)44100.0,	/* Sampling Frequency */
							bassdrum,	/* Contains bass drum hits and levels */
							snaredrum,	/* Contains snare drum hits and levels */
							hihat		/* Contains hihat hits and levels */
							) != OKAY)
				return(NOT_OKAY);
/*
