								   ...
								   struct hardwareMeterValType *sp_meters, int DSP_data_type)
{
	...
	if( !(s->bypass_on) )
	{
			...
			for(i=0; i<l_length; i++)
			{
				...
				{	
					/* 200 hz Butterworth single pole, no zero */
					#define	BETA 0.9719032384
					#define LPGAINCOMP (1.0 - BETA)

					static float lp_out_one_old = 0.0;
					static float lp_out_two_old = 0.0;

					float lp_out_one, lp_out_two, hp_out;
					...
					*/
					...
					lp_out_one = mono * LPGAINCOMP + BETA * lp_out_one_old;
					lp_out_one_old = lp_out_one;

					lp_out_two = lp_out_one * LPGAINCOMP + BETA * lp_out_two_old;
					lp_out_two_old = lp_out_two;

					out1 = mono - lp_out_two;
					out2 = out1;
