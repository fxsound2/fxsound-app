...
/* Highpass filters for stereo syn (second order) */
static float xv_hp1[3];
static float xv_hp2[3];
static float yv_hp1[3];
static float yv_hp2[3];
...
#define FIRST_VERSION
...
#ifdef FIRST_VERSION
				{
					#define HP_SYN_GAIN   1.008092219e+00
				...
				{
					/* Currently a 80 hz 2 pole butterworth highpass */
					/* Note that  having the highpass filter in the feedback loop
					 * modified the stereo spreading in a bad way. Fixed the
					 * bass problem that the pure comb filters have though.
					 */
					xv = &(xv_hp1[0]);
					yv = &(yv_hp1[0]);
					xv[0] = xv[1]; xv[1] = xv[2]; 
					xv[2] = l_del[d_index] * (realtype)(1.0 / HP_SYN_GAIN);
					yv[0] = yv[1]; yv[1] = yv[2]; 

				   yv[2] =   (xv[0] + xv[2]) - 2 * xv[1]
									+ ( -0.9840099176 * yv[0]) + (  1.9838810417 * yv[1]);

        			filt1 = yv[2];
					out1 = mono + d_gain * filt1;
					l_del[d_index] = out1;

					xv = &(xv_hp2[0]);
					yv = &(yv_hp2[0]);
					xv[0] = xv[1]; xv[1] = xv[2]; 
					xv[2] = r_del[d_index] * (realtype)(1.0 / HP_SYN_GAIN);
					yv[0] = yv[1]; yv[1] = yv[2];
