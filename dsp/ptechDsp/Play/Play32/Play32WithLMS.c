	...
	if( !(s->bypass_on) )
	{
		if( (s->vocal_elim_on) && (s->vocal_elim_val != 0) )
		{
			...
			for(i=0; i<l_length; i++)
			{
				...
				N = 128;
				mu = 0.05;
				mu_n = 0.01; /* 128 weights, 0.1, 0.05, 0.025 distorts, 0.01 is ok */

#ifdef UNDEF
				afiltLMS(	in1_bp,		/* Desired input */
								in2_bp,		/* Filter input */
								mu,		/* Filtering time constant */
								&diff,	/* Pre-weight update formed error */
								&XW,	/* Pre-weight update filter output */
								W,	/* Pointed by function to internal static weight values */
								N,			/* Filter order */
								0,	/* Delay added by function to filter input signal */
								0		/* Delay added by function to filter desired signal */
						  );

				afiltNLMS(	in1_bp,		/* Desired input */
								in2_bp,		/* Filter input */
								mu_n,		/* Filtering time constant */
								&diff_n,	/* Pre-weight update formed error */
								&XW,	/* Pre-weight update filter output */
								W,	/* Pointed by function to internal static weight values */
								N,			/* Filter order */
								0,	/* Delay added by function to filter input signal */
								2		/* Delay added by function to filter desired signal */
						  );

				out1 = in1_bs + diff;
				out2 = in1_bs + diff_gain * diff_n;
#endif
