...
/* Variables for simulated stereo */
#define STEREO_MAX_DELAY_LEN 4096
static float l_del[STEREO_MAX_DELAY_LEN];
static float r_del[STEREO_MAX_DELAY_LEN];
								   ...
								   struct hardwareMeterValType *sp_meters, int DSP_data_type)
{
	...
	static float d_gain = 0.495;
	static int l_delay = 981;
...
#define FIRST_VERSION
...
#ifdef DSP_READ_VALS
				if( ReadVals.switch_vals[0] ) /* Functions as bypass for now */
#else
				if( 1 )
#endif
#ifdef FIRST_VERSION
				{
					/* Note reasonable values at 44100 hz. are d_gain = 0.495
					 * l_delay = 981. Works well with DFX standard preset with
					 * 3D boosted to mid 4 and Dynamic Gain boosted to mid 5
					 */
					/* NOTES- basic algorithm has the problem that one channel will get bass peaks
					 * while the other channel will get bass dips. Tried using a lp filter in the
					 * feedback loop but spreading was decreased. Tried using Butterworth and Bessel
					 * lp and hp filters to split and recombine but slight midrange boost occurred.
					 * Below am using a simple one-pole filter that when differenced with input
					 * creates a highpass with a zero at zero hz. Note that for this recombining to
					 * work the lowpass had to be implemented without the zero at -1 that occurs when
					 * using the bilinear transform on an analog one pole filter. May be able to derive higher order
					 * versions of this.
					/*
					...
					#ifdef DSP_READ_VALS
					if( ReadVals.switch_vals[1] ) /* Functions as bypass for now */
					#else
					if( 1 )
					#endif
					{
						/* Pole at 200 hz. */
						/* Note - compared to hand tweaking this value seems pretty good */
						#define LP_BETA 0.9719032384
						#define LP_GAINCOMP (1.0 - LP_BETA)

						realtype low_pass, high_pass;
						realtype filt1, filt2;
						static realtype low_pass_old = (realtype)0.0;
						realtype beta;

						/* beta = ReadVals.f_vals[2]; */

						/* Currently a simple 200 hz 1 pole filter that differences into a highpass */
						low_pass = mono * LP_GAINCOMP + LP_BETA * low_pass_old;
						/* low_pass = mono * ((realtype)1.0 - beta) + beta * low_pass_old; */
						low_pass_old = low_pass;

						/* For this simple filter, this differencing puts a zero at zero hz. */
						high_pass = mono - low_pass;

						filt1 = high_pass + d_gain * l_del[d_index];
						l_del[d_index] = filt1;

						filt2 = high_pass - d_gain * r_del[d_index];
						r_del[d_index] = filt2;
					...
					{
						/* Version with no low pass, was causing bass boost on right channel, cut on left */
						out1 = mono + d_gain * l_del[d_index];
						l_del[d_index] = out1;

						out2 = mono - d_gain * r_del[d_index];
						r_del[d_index] = out2;
				...
				// Second version is showing some promise, potentially same spread with less resonance
				// Use alternating gain on delay elements
				//
				...
				// Pure FIR
				{
					...
					for(j=0; j<4; j++)
					{
						delay[j] = ReadVals.l_vals[j * 2];

						gain[j] = ReadVals.f_vals[j * 2 + 1];

						/* Invert gain if switch is off */
						if( ReadVals.switch_vals[j * 2 + 1] == 0 )
							gain[j] = - gain[j];

						index = d_index - delay[j];

						if( index < 0 )
							index += STEREO_MAX_DELAY_LEN;

						out1 += l_del[index] * gain[j];
						out2 -= r_del[index] * gain[j];
					}
					...
					if(d_index >= STEREO_MAX_DELAY_LEN)
						d_index = 0;
				}
		...
		if( s->bassboost_on )
		{
			...
			for(i=0; i<l_length; i++)
			{
				...
				if(s->stereo_in_flag)
				{
					...
					/* Note that both out1 and out2 are used in mono case, so out2 must be assigned */
					out2 = out1;
...
{ 
	...
	yv[8] =   (xv[0] + xv[8]) -   7.8839668614 * (xv[1] + xv[7]) +  27.3088500520 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8622629891 * (xv[1] + xv[7]) +  27.1806922410 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8345499343 * (xv[1] + xv[7]) +  27.0175647530 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8363539694 * (xv[1] + xv[7]) +  27.0281663250 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.7629769950 * (xv[1] + xv[7]) +  26.5989294340 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8800903508 * (xv[1] + xv[7]) +  27.2859339760 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8310172916 * (xv[1] + xv[7]) +  26.9968119330 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8747059787 * (xv[1] + xv[7]) +  27.2541228440 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8232866002 * (xv[1] + xv[7]) +  26.9514299610 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8669727040 * (xv[1] + xv[7]) +  27.2084723220 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8122434771 * (xv[1] + xv[7]) +  26.8866805540 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8536815259 * (xv[1] + xv[7]) +  27.1301175660 * (xv[2] + xv[6])
...
{ xv[0] = xv[1]; xv[1] = xv[2]; xv[2] = xv[3]; xv[3] = xv[4]; xv[4] = xv[5]; xv[5] = xv[6]; xv[6] = xv[7]; xv[7] = xv[8]; 
  ...
  yv[0] = yv[1]; yv[1] = yv[2]; yv[2] = yv[3]; yv[3] = yv[4]; yv[4] = yv[5]; yv[5] = yv[6]; yv[6] = yv[7]; yv[7] = yv[8]; 
  yv[8] =   (xv[0] + xv[8]) -   7.8700365903 * (xv[1] + xv[7]) +  27.2265534750 * (xv[2] + xv[6])
