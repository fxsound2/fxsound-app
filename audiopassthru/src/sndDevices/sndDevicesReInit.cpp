	...
	*ipDfxDeviceEnabledFlag = IS_FALSE;
	...
	if( (i_initType == SND_DEVICES_INIT_FOR_PROCESSING))
	{
		if (cast_handle->numRealDevices > 0) 
		{
			...
			// Initialize the capture device's (DFX) volume to the playback device's own volume, so the
			// FxSound control always mirrors whichever real device is currently selected.
