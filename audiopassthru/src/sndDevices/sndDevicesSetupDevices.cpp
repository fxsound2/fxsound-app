...
/*
 * FUNCTION: sndDevices_RestoreSavedPlaybackDeviceSettings()
 * Puts the saved volume level and mute setting back on the real playback device they were saved from.
 * Called whenever FxSound stops driving that device, either because it is switching to a different
 * playback device, or because it is handing control back to the real device on exit or power off.
 * The device is looked up by its ID in the current device list rather than held as an object pointer,
 * since all device objects are released and re-created on every re-init. If the device is no longer
 * present there is nothing to restore, its settings went away with it.
 * b_never_raise_volume limits the volume write so that it can only lower the device, see below.
 */
static void sndDevices_RestoreSavedPlaybackDeviceSettings(struct sndDevicesHdlType *cast_handle,
                                                          BOOL b_never_raise_volume)
{
	IMMDevice *pDevice;
	...
	if( sndDevices_UtilsGetIndexFromID((PT_HANDLE *)cast_handle, cast_handle->savedPlaybackDeviceID, &deviceIndex) != OKAY )
		return;

	if( (deviceIndex < 0) || (deviceIndex >= SND_DEVICES_MAX_NUM_DEVICES) )
		return;

	pDevice = cast_handle->pAllDevices[deviceIndex];
	if( pDevice == NULL )
		return;

	hr = pDevice->Activate(cast_handle->IID_IAudioEndpointVolume, CLSCTX_ALL, NULL, (void**)&pEndptVol);
	if( FAILED(hr) || (pEndptVol == NULL) )
		return;

	// When FxSound is switching away, the device is about to go silent, so its own level can be put back
	// as it was, a raise included. When FxSound is handing control back to it, that device is about to
	// become the one the user hears, and restoring a level higher than the one FxSound was running at
	// would step the volume up at exactly the wrong moment, so never raise it in that case.
	// This also covers the unmute: when the device's own setting is unmuted and FxSound was muted, the
	// device comes back unmuted but at no more than the level FxSound was already playing at.
	float targetVolume = cast_handle->savedPlaybackVolume;

	if( b_never_raise_volume )
	{
		float currentVolume;

		if( SUCCEEDED(pEndptVol->GetMasterVolumeLevelScalar(&currentVolume)) && (targetVolume > currentVolume) )
			targetVolume = currentVolume;
	}

	// Note, second arg is a GUID used in callbacks generated by this change to identify who made the change.
	pEndptVol->SetMasterVolumeLevelScalar(targetVolume, &(cast_handle->guidThisApplication));
	pEndptVol->SetMute(cast_handle->savedPlaybackMute, &(cast_handle->guidThisApplication));
...
int PT_DECLSPEC sndDevicesInitialSetupPlaybackDevice(PT_HANDLE *hp_sndDevices, int *ip_status)
{
	...
	*ip_status = SND_DEVICES_DEVICE_OPERATION_COMPLETED;
	...
	if( cast_handle->initializationMode == SND_DEVICES_INIT_FOR_PROCESSING )
	{
		...
		// If we are moving onto a different playback device, hand the device we are leaving its own
		// volume level and mute setting back, then take a copy of the new devices settings before
		// anything in this app gets a chance to change them.
		// Note this must happen before the volume and mute syncing done at the end of sndDevicesReInit().
		if( (cast_handle->playbackDeviceNum >= 0) && (cast_handle->playbackDeviceNum < SND_DEVICES_MAX_NUM_DEVICES) )
		{
			wchar_t *playbackDeviceID = cast_handle->pwszID[cast_handle->playbackDeviceNum];
			float playbackVolSetting;
			BOOL playbackMuteSetting;

			if( cast_handle->playbackSettingsAreSaved && (wcscmp(cast_handle->savedPlaybackDeviceID, playbackDeviceID) != 0) )
			{
				// Switching away, so the device is going quiet - put its own level back as it was.
				sndDevices_RestoreSavedPlaybackDeviceSettings(cast_handle, FALSE);
			...
			if( !cast_handle->playbackSettingsAreSaved )
			{
				if( SUCCEEDED(cast_handle->pEndptVolPlayback->GetMasterVolumeLevelScalar(&playbackVolSetting)) &&
					 SUCCEEDED(cast_handle->pEndptVolPlayback->GetMute(&playbackMuteSetting)) )
				{
					wcscpy(cast_handle->savedPlaybackDeviceID, playbackDeviceID);
					cast_handle->savedPlaybackVolume = playbackVolSetting;
					cast_handle->savedPlaybackMute = playbackMuteSetting;
					cast_handle->playbackSettingsAreSaved = TRUE;
				}
			}
		}
...
int PT_DECLSPEC sndDevicesRestoreDefaultDevice(PT_HANDLE *hp_sndDevices, int *ip_resultFlag)
{
	...
	*ip_resultFlag = SND_DEVICES_DEVICE_OPERATION_COMPLETED;
	...
	// Handing control back to a real device is an output device switch, in the same sense that Windows
	// treats picking a different output device, so that device's own mute setting and volume level
	// apply from here on - not FxSound's. Done before the default device is changed below, so the
	// device already carries its own settings by the time the user is listening through it.
	// Note the FxSound mute setting is not lost by this: it stays on the DFX device, which Windows
	// remembers, so muting FxSound and restarting it comes back muted.
	// Note also that playbackSettingsAreSaved is deliberately left set. On a power off the process
	// carries on running, and these saved values are still the device's true own settings. Clearing
	// the flag would let the next attach re-save whatever the restore just left on the device, which
	// after a clamped volume restore is not the device's own level.
	sndDevices_RestoreSavedPlaybackDeviceSettings(cast_handle, TRUE);
